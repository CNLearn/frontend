...
describe("SearchInput", () => {
  ...
  test("Checks if inputting values works", async () => {
    ...
    // let's find the InputText box and set its content
    const searchBox = wrapper.find("#search_box");
    // now let's input some text inside it
    await searchBox.setValue("你好");
    ...
    const searchString = wrapper.find('[data-test="searchString"]').element.textContent;
  ...
  // this one also has to get updated
  test("Checks if anything that is not a Chinese character gets filtered out", async () => {
    ...
    await wrapper.setData({ localString: "你们是our friends" });
    // let's wait for the actions to happen
    await flushPromises();
